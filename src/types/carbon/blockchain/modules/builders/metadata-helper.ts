...
export function pushMetadataField(
  fieldSchema: VmNamedVariableSchema,
  metadata: VmDynamicStruct,
  metadataFields: MetadataField[]
): void {
  let found = metadataFields.find((f) => f.name === fieldSchema.name.data);
  if (!found) {
    found = metadataFields.find(
      (f) => f.name.toLowerCase() === fieldSchema.name.data.toLowerCase()
    );
    if (found) {
      throw Error(
        `Metadata field '${fieldSchema.name.data}' provided in incorrect case: '${found.name}'`
      );
    }
  ...
  metadata.fields.push(
    VmNamedDynamicVariable.from(fieldSchema.name, fieldSchema.schema.type, normalizedValue)
  );
}
...
type MetadataValueArray = MetadataValueInput[];
  ...
  switch (type) {
    ...
    case VmType.Int8:
      return Uint8Array.from(
        values.map(
          (val, idx) => ensureIntegerInRange(elementPath(idx), val, -0x80, 0x7f, 0xff) & 0xff
        )
      );
    case VmType.Int16:
      return values.map((val, idx) =>
        ensureIntegerInRange(elementPath(idx), val, -0x8000, 0x7fff, 0xffff)
      );
    case VmType.Int32:
      return values.map((val, idx) =>
        ensureIntegerInRange(elementPath(idx), val, -0x80000000, 0x7fffffff, 0xffffffff)
      );
    case VmType.Int64:
      return values.map((val, idx) =>
        ensureBigInt(elementPath(idx), val, INT64_MIN, INT64_MAX, 'Int64', UINT64_MAX)
      );
    case VmType.Int256:
      return values.map((val, idx) =>
        ensureBigInt(elementPath(idx), val, INT256_MIN, INT256_MAX, 'Int256', UINT256_MAX)
      );
    case VmType.Bytes:
    ...
    case VmType.Struct: {
      ...
      sa.structs = values.map((val, idx) =>
        normalizeStructValue(elementPath(idx), structSchema, val as MetadataStructInput)
      );
      return sa;
    }
    default:
      throw Error(
        `Metadata field '${fieldName}' with type '${VmType[type] ?? type}' does not support array values`
      );
  }
}

function normalizeStructValue(
  fieldName: string,
  structSchema: VmStructSchema | undefined,
  value: MetadataStructInput
): VmDynamicStruct {
  if (!structSchema) {
  ...
  structSchema.fields.forEach((childSchema) => {
    ...
    if (!childValue) {
      childValue = providedFields.find(
        (f) => f.name.toLowerCase() === childSchema.name.data.toLowerCase()
      );
      if (childValue) {
        throw Error(
...
function metadataStructInputToFields(
  fieldName: string,
  value: MetadataStructInput
): MetadataField[] {
  if (Array.isArray(value)) {
...
function ensureBigInt(
  ...
  if (typeof value === 'bigint') {
    ...
    if (!Number.isSafeInteger(value)) {
      throw Error(
        `Metadata field '${fieldName}' must be provided as a bigint when it exceeds safe integer range`
      );
    }
