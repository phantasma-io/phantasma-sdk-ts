 ...
 * only for version >= 1. This mirrors the chain's wire format exactly. The version-0 byte image is
 * frozen forever for historical replay. A version>=1 image truncated to the v0 length fails to
 * parse, because the tail read throws on end of stream.
 */
